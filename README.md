...
Database Either let the resource create tables automatically on start, or run sql/vehicle_runs.sql manually in your MySQL database.

Configuration options to tune RUN_TIMEOUT in server.lua — how long a run lasts before despawn.
...
Admin commands /spawnrun — spawn a test run (admin only).

/vehraid_autotoggle — toggle automatic spawns (admin only).

/vehraid_autostatus — show auto-spawn status. Troubleshooting Vehicles not visible to all players: ensure vehicles are created as mission entities and networked (VehToNet used).
...
Optional improvements Replace native notifications with your preferred QBCore notification wrapper.
...
Installation
Place the resource folder in your resources directory (e.g., resources/[local]/vehicle_runs).
Ensure fxmanifest.lua, server.lua, client.lua, and config.lua are present.
